...
add_action('admin_notices', 'wp_rem_cs_show_server_requirements_warning');
...
if ( ! function_exists( 'wp_rem_cs_show_server_requirements_warning' ) ) {
	function wp_rem_cs_show_server_requirements_warning() {
		$screen = get_current_screen();
		if ( $screen->base == 'toplevel_page_wp_rem_cs_demo_importer'){
			if ( false === ( $admin_dismiss_notice = get_transient('admin_dismiss_notice') ) ) {
				$class = 'notice server-requirements notice-warning is-dismissible';
				$message = __('Your server requirements is not enough for import demo data.', 'wp-rem-frame');
					...
					<div class="importer-steps-containers-wrapper importer-text row">
						<div class="step-1-container">
							<div class="col-lg-6 col-md-6 col-sm-12">
								...
								<ul class="importer-requirement import-server-requirements">
							...
							<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
								...
								if ( $envato_purchase_code_verification ) {
									...
									if (
										...
										$verify_code = false;
										if ( function_exists( 'wp_rem_update_demo_data' ) ) {
											wp_rem_update_demo_data();
										}
									}
                                                                        }
								}
								...
								<?php if ( $verify_code ) : ?>
									<form role="form">
										<div class="control-group">
											<label for="item-purchase-code"><?php _e( 'Envato Provided Item Purchase Code', 'wp-rem-frame' ); ?></label>
											...
											<label class="purchase-code-exists-error" style="color: #ff0000; display: none;"><?php _e( 'The Purchase Code is being used on another site. Please deregister it from there and then try again.', 'wp-rem-frame' ); ?></label>

											<label for="envato-email-address"><?php _e( 'Envato Provided Email Address', 'wp-rem-frame' ); ?></label>
								...
								<?php else : ?>
									...
									<input type="button" value="<?php _e( 'Deregister Purchase Code', 'wp-rem-frame' ); ?>" class="importer-next-btn release-purchase-code"><br><br><br>
								<?php endif; ?>
							</div>
						</div>
								...
								<div class="sub-step-2 col-lg-5 col-md-5 col-sm-12 col-xs-12 hidden">
									...
									<ul class="wp-rem-select-demo">
										<li class="content-chk-wrapper following-pages hidden">
											<div class="radiobox">
												<input type="checkbox" id="chk-wp-content-new" value="content_new" name="chk-wp-content-new" class="checkbox-data-import-type">
												<label for="chk-wp-content-new" class="checkbox-data-import-type"><?php _e( 'WP Content ( Pages, Posts, etc. )', 'wp-rem-frame' ); ?></label>
											</div>
										</li>
										<li class="content-chk-wrapper following-pages hidden">
											<div class="radiobox">
										...
										<li class="fonts-chk-wrapper hidden">
											<div class="radiobox">
												...
												<label for="chk-fonts" class="checkbox-data-import-type"><?php _e( 'Fonts', 'wp-rem-frame' ); ?></label>	
											</div>
										</li>
										<li class="icons-chk-wrapper hidden">
											<div class="radiobox">
												...
												<label for="chk-icons" class="checkbox-data-import-type">
													<?php _e( 'Icons', 'wp-rem-frame' ); ?>
													<?php echo $is_cs_icons_manager_installed ? '' : '<br>' . __( 'Please install CS Icons Manager Plugin.', 'wp-rem-frame' ); ?>
												</label>	
											</div>
										</li>
										...
										<li class="menus-chk-wrapper hidden">
											<div class="radiobox">
												...
												<label for="chk-menus" class="checkbox-data-import-type"><?php _e( 'Menus Locations', 'wp-rem-frame' ); ?></label>
											</div>
										</li>
				...
				$(function ( ) {
						...
						if (current_step == 1) {
							...
							if (selected_demo == "") {
								...
								if (current_sub_step == 1) {
									...
									var data_types = {
										"content_new": ["content_new", "WP Content ( posts, pages, etc. )"],
					...
					function add_to_active_themes() {
						$.ajax({
							...
							"url": "<?php echo admin_url( 'admin-ajax.php' ); ?>",
							"data": {
								"action": "wp_rem_add_to_active_themes",
								"selected_demo": selected_demo,
							},
							"dataType": "json",
							"success": function (data) {
					...
					$('.release-purchase-code').click(function ( ) {
						$.ajax({
							...
							"url": "<?php echo admin_url( 'admin-ajax.php' ); ?>",
							"data": {
								"action": "wp_rem_release_purchase_code",
							},
							"dataType": "json",
							"success": function (data) {
								location.reload();
					...
					if(jQuery('.import-server-requirements').find('i.error').length > 0 ){
						$(".notice.server-requirements").show();
					}
				});
			})(jQuery);
		</script>
...
function wp_rem_cs_import_demo_data_callback() {
	...
	if ( 'import_data' == $action_type ) {
		...
		if ( 'content_new' == $import_type ) {
			...
			$wp_rem_cs_importer->wp_data_path = isset( $urls['content-new'] ) ? $urls['content-new'] : '';
			$wp_rem_cs_importer->is_content = true;
		} elseif ( 'navitems' == $import_type ) {
			$wp_rem_cs_importer->wp_data_path = isset( $urls['navitems'] ) ? $urls['navitems'] : '';
			$wp_rem_cs_importer->is_navitems = true;
		} elseif ( 'media_attachments' == $import_type ) {
			$wp_rem_cs_importer->wp_data_path = isset( $urls['media-attachments'] ) ? $urls['media-attachments'] : '';
			$wp_rem_cs_importer->attachments_path = isset( $urls['attachments'] ) ? $urls['attachments'] : '';
			$wp_rem_cs_importer->is_attachments_zip = true;
			$wp_rem_cs_importer->is_media_attachments = true;
			...
			$wp_rem_cs_importer->fonts_data_path = isset( $urls['fonts'] ) ? $urls['fonts'] : '';
			$wp_rem_cs_importer->is_fonts = true;
		} elseif ( 'icons' == $import_type ) {
			$wp_rem_cs_importer->icons_data_path = isset( $urls['icons'] ) ? $urls['icons'] : '';
			$wp_rem_cs_importer->is_icons = true;
		...
		$envato_email_address = isset( $_POST['envato_email_address'] ) ? $_POST['envato_email_address'] : '';
                ...
                $envato_purchase_code_verification['last_verification_time'] = isset( $envato_purchase_code_verification['last_verification_time'] )? $envato_purchase_code_verification['last_verification_time'] : 0;
		...
		if ( isset( $verify_response['body'] ) && $verify_response['body'] < 1 ) {
			...
			//echo json_encode(array( 'success' => false, 'message' => __('The purchase code is being used on anoher site.', 'wp-rem-frame') ));
		...
		$demo_data_name = isset( $_POST['item_demo_data_slug'] ) ? $_POST['item_demo_data_slug'] : '';
                ...
                wp_rem_add_to_active_themes_callback($demo_data_name, 'incomplete');
...
if ( ! function_exists( 'wp_rem_update_demo_data' ) ) {
	...
	function wp_rem_update_demo_data() {
		$envato_purchase_code_verification = get_option( 'item_purchase_code_verification' );
...
if ( ! function_exists( 'wp_rem_delete_default_content_callback' ) ) {
	add_action( 'wp_rem_delete_default_content', 'wp_rem_delete_default_content_callback' );
	function wp_rem_delete_default_content_callback() {

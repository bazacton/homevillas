...
if (!class_exists('wp_rem')) {
	...
	public $plugin_url;
	public $plugin_dir;
	public static $wp_rem_version;
	public static $wp_rem_data_update_flag;
	...
	public function __construct() {
	    ...
	    self::$wp_rem_version = '1.9';
	    self::$wp_rem_data_update_flag = 'wp_rem_old_data_update_flag_' . str_replace(".", "_", self::$wp_rem_version);
	    ...
	    add_action('admin_notices', array($this, 'check_db_update_property_visibility'), 5);
	    add_action('wp_ajax_wp_rem_property_visibility_db_update', array($this, 'wp_rem_property_visibility_db_update_callback'));
	    add_action('wp_ajax_nopriv_wp_rem_property_visibility_db_update', array($this, 'wp_rem_property_visibility_db_update_callback'));
	    add_action('wp_rem_importer_db_update', array($this, 'wp_rem_importer_db_update_callback'));
	}
	...
	private static function update_wp_rem_plugin_version() {
	    if ((get_option('wp_rem_plugin_version') == '') || ( get_option('wp_rem_plugin_version') !== wp_rem::get_plugin_version() )) {
		delete_option('wp_rem_plugin_version');
		add_option('wp_rem_plugin_version', wp_rem::get_plugin_version());
	    }
	}
	...
	public function wp_rem_property_visibility_db_update_callback() {
	    $args_count = array(
		'posts_per_page' => "-1",
	    ...
	    $msg = '';
	    $property_loop_obj = wp_rem_get_cached_obj('property_visibility_all_result_cached_loop_obj', $args_count, 12, false, 'wp_query');
	    if ($property_loop_obj->have_posts()) {
		while ($property_loop_obj->have_posts()) : $property_loop_obj->the_post();
		    global $post;
		    $property_id = $post;
		    $wp_rem_property_member = update_post_meta($property_id, 'wp_rem_property_visibility', 'public');
		endwhile;
	    }
	    do_action('wp_rem_plugin_db_structure_updater');
	    add_option(self::$wp_rem_data_update_flag, 'yes');
	    $json['type'] = 'success';
	    $json['msg'] = wp_rem_plugin_text_srt('wp_rem_property_visibility_updated_msg');
	    echo json_encode($json);
	    die();
	}

	/*
	 * importer update all data for property visibility
	 */

	public function wp_rem_importer_db_update_callback() {
	    $args_count = array(
		'posts_per_page' => "-1",
		'post_type' => 'properties',
		'post_status' => 'publish',
	...
	public static function check_db_update_property_visibility() {
	    global $wp_rem_Class;
	    ...
	    if (get_option(self::$wp_rem_data_update_flag) !== 'yes') {
									...
									$(".btnConfirmVisiblePropertyUpgrade").click(function() {
										...
										$(".confirm-db-update-btn").click(function() {
											...
											$.ajax({
												...
												data: "envato_email="+envato_email+"&theme_demo="+theme_demo+"&action=wp_rem_property_visibility_db_update",
		...
		$message = '<h2>Homevillas Alert!</h2>';
	...
	public function wp_rem_loader() {
	    ...
	    echo '<div class="loader-img"><i class="icon-spinner8"></i></div></div>';
		...
		if (!wp_rem::is_demo_user_modification_allowed()) :
		    ...
		    <script type="text/javascript">
		    	...
		    	function bind_rest_auth_event() {
		    	    $("input[type='submit'], .btn-submit, .btn-send, .review-reply-btn, .review-property .delete-this-user-review, .delete-shortlist, .remove_member, #team_update_form, .wp-rem-dev-property-delete, .discussion-submit, .viewing-request-holder input, .delete-hidden-property, .property-visibility-update, .sold-property-box label, #send_your_review").off("click");
		    	    $(document).off("click", "input[type='submit'], .btn-submit, .btn-send, .review-reply-btn, .review-property .delete-this-user-review, .delete-shortlist, .remove_member, #team_update_form, .wp-rem-dev-property-delete, .discussion-submit, .viewing-request-holder input, .delete-hidden-property, .property-visibility-update, .sold-property-box label, #send_your_review");
		    	    ...
		    	    $("body").on("click", "input[type='submit'], .btn-submit, .btn-send, .review-property .delete-this-user-review, .delete-shortlist, .remove_member, #team_update_form, .wp-rem-dev-property-delete, .discussion-submit, .viewing-request-holder input, .delete-hidden-property, .property-visibility-update, .sold-property-box label, .review-reply-btn, #send_your_review", function (e) {
		    		e.stopPropagation();
		    		e.preventDefault();
		    		e.stopImmediatePropagation();
		    		jQuery.growl.error({
		...
		if (!wp_rem::is_demo_user_modification_allowed_frontend()) :
		    ?>
		    <script type="text/javascript">
		        (function ($) {
		    	$(document).ready(function () {
		    	    ...
		    	    $('.property-hide-opt .hide-btn').attr('onclick', '');
		    	    //$('.property-note-opt .property-notes')
		    	});

		    	function bind_rest_auth_event_frontend() {
		    	    $(".viewing-request-holder input[type='submit'], .enquiry-request-holder input[type='submit'], .property-hide-opt .hide-btn, .submit-prop-notes, #send_your_review").off("click");
		    	    $(document).off("click", ".viewing-request-holder input[type='submit'], .enquiry-request-holder input[type='submit'], .property-hide-opt .hide-btn, .submit-prop-notes, #send_your_review");
		    	    $("body").off("click", ".viewing-request-holder input[type='submit'], .enquiry-request-holder input[type='submit'], .property-hide-opt .hide-btn, .submit-prop-notes, #send_your_review");
		    	    $("body").on("click", ".viewing-request-holder input[type='submit'], .enquiry-request-holder input[type='submit'], .property-hide-opt .hide-btn, .submit-prop-notes, #send_your_review", function (e) {
		    		e.stopPropagation();
		    		e.stopImmediatePropagation();
		    		jQuery.growl.error({
		    		    message: '<?php echo wp_rem_plugin_text_srt('wp_rem_demo_user_not_allowed_to_modify'); ?>'
	...
	public function includes() {
	    ...
	    require_once 'backend/classes/email-templates/class-update-viewing-status-template.php';
	    require_once 'backend/classes/email-templates/class-property-claim-template.php';
	    require_once 'backend/classes/email-templates/class-flag-property-template.php';
	    require_once 'backend/classes/email-templates/class-confirm-reset-password-template.php';
	    require_once 'backend/classes/email-templates/class-property-claim-status-template.php';
	    require_once 'backend/classes/email-templates/class-property-flag-status-template.php';
	    /*
	     * croppic
	     */
	    ...
	    require_once 'backend/post-types/properties/classes/class-properties-faqs.php';
	     ...
	     * Property FAQ'S
	     */
	    require_once 'frontend/classes/class-properties-faqs.php';
	    ...
	    add_action('wp_enqueue_scripts', array($this, 'wp_rem_enqueue_responsive_front_scripts'), 10);
	...
	public function load_plugin_textdomain() {
	    ...
	    load_plugin_textdomain('wp-rem', false, plugin_basename(dirname(__FILE__)) . '/languages');
	}
	...
	public static function get_plugin_version() {
	    $plugin_data = self::$wp_rem_version;
	    return $plugin_data;
	}
	...
	public function wp_rem_defaultfiles_plugin_enqueue() {
	    ...
	    wp_register_script('wp_rem_init_js', plugins_url('/assets/frontend/scripts/init.js', __FILE__), array('jquery'), wp_rem::get_plugin_version(), true);
	    wp_register_script('wp_rem_encryption_js', plugins_url('/assets/frontend/scripts/wp-rem-encryption.js', __FILE__), '', wp_rem::get_plugin_version(), true);
	    /*
		...
		if (is_admin()) {
		    // admin js files
		    $wp_rem_cs_scripts_path = plugins_url('/assets/backend/scripts/cs-page-builder-functions.js', __FILE__);
		    wp_enqueue_script('cs-frame-admin', $wp_rem_cs_scripts_path, array('jquery'));
		}
		wp_enqueue_style('wp-rem-default-element-style', plugins_url('/assets/frontend/css/default-element.css', __FILE__));


		wp_enqueue_script('wp-rem-resize-sensor', plugins_url('/assets/frontend/scripts/ResizeSensor.js', __FILE__), '', '');
		wp_enqueue_script('wp-rem-element-queries', plugins_url('/assets/frontend/scripts/ElementQueries.js', __FILE__), '', '');
	    }
	    /*
	    ...
	    wp_register_script('wp-rem-property-categories', plugins_url('/assets/common/js/property-categories.js', __FILE__), array('jquery'), wp_rem::get_plugin_version());
	    ...
	    //wp_register_script('jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.11.1/jquery-ui.js');
            wp_register_script('jquery-ui', 'https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js');
            
	    $wp_rem_pt_array = array(
		'plugin_url' => wp_rem::plugin_url(),
	    );
	    wp_localize_script('chosen-ajaxify', 'wp_rem_chosen_vars', $wp_rem_pt_array);
	    wp_enqueue_script('cs_locations_functions_js', plugins_url('/assets/frontend/scripts/jquery.location-autocomplete.js', __FILE__), '', '', true);
	    $cs_pt_array = array(
		'address' => esc_html__('Address', 'wp-rem'),
		'country' => esc_html__('Country', 'wp-rem'),
		'state' => esc_html__('State', 'wp-rem'),
		'city' => esc_html__('City', 'wp-rem'),
		'town' => esc_html__('Town', 'wp-rem'),
                'plugin_dir_url' => wp_rem::plugin_url(),
	    );
	    wp_localize_script('cs_locations_functions_js', 'cs_vars', $cs_pt_array);
		...
		wp_enqueue_style('wp-rem-widget', plugins_url('/assets/frontend/css/widget.css', __FILE__));
		wp_enqueue_style('wp_rem_plugin_css', plugins_url('/assets/frontend/css/wp-rem-plugin.css', __FILE__), '', wp_rem::get_plugin_version());


		if (!class_exists('wp_rem_real_estate_framework')) {
		    wp_enqueue_style('wp-rem-element-queries', plugins_url('/assets/frontend/css/elementqueries.css', __FILE__), '', wp_rem::get_plugin_version());
		}
	     ...
	     * New Scripts
	     */
	    wp_register_script('wp-rem-validation-script', plugins_url('/assets/frontend/scripts/wp-rem-validation.js', __FILE__), '', wp_rem::get_plugin_version(), true);
	    wp_register_script('wp-rem-members-script', plugins_url('/assets/frontend/scripts/wp-rem-members.js', __FILE__), '', wp_rem::get_plugin_version(), true);
	    wp_register_script('wp-rem-login-script', plugins_url('/assets/frontend/scripts/wp-rem-login.js', __FILE__), '', wp_rem::get_plugin_version(), true);
	    wp_register_script('wp-rem-icons-loader', plugins_url('/assets/common/js/icons-loader.js', __FILE__), array('jquery'));
	    wp_register_script('wp-rem-property-functions', plugins_url('/assets/frontend/scripts/property-functions.js', __FILE__), array('jquery'), wp_rem::get_plugin_version());
	    wp_register_script('wp-rem-hide-property-functions', plugins_url('/assets/frontend/scripts/hide-property-functions.js', __FILE__), array('jquery'), wp_rem::get_plugin_version());
	    wp_register_script('jquery-mixitup', plugins_url('/assets/frontend/scripts/jquery.mixitup.min.js', __FILE__), array('jquery'));
	    wp_register_script('wp-rem-member-functions', plugins_url('/assets/frontend/scripts/member-functions.js', __FILE__), array('jquery'), wp_rem::get_plugin_version());
	    $wp_rem_property_functions_string = array(
		'property_type' => wp_rem_plugin_text_srt('wp_rem_rem_property_type'),
		'property_cats' => wp_rem_plugin_text_srt('wp_rem_property_categories_label'),
		'datepicker_apply' => wp_rem_plugin_text_srt('wp_rem_rem_property_type'),
		'add_prop_phone_num' => wp_rem_plugin_text_srt('wp_rem_enquire_arrange_viewing_phone_number'),
		'datepicker_cancel' => wp_rem_plugin_text_srt('wp_rem_property_datepicker_cancel'),
		...
		'wp_rem_edit_details' => wp_rem_plugin_text_srt('wp_rem_edit_details'),
		'wp_rem_edit_details_update' => wp_rem_plugin_text_srt('wp_rem_edit_details_update'),
		'wp_rem_edit_details_edit' => wp_rem_plugin_text_srt('wp_rem_edit_details_edit'),
	    );
	    // temprary off

	    if (!is_admin()) {
		wp_enqueue_script('wp_rem_functions_frontend', plugins_url('/assets/frontend/scripts/functions.js', __FILE__), '', wp_rem::get_plugin_version());
		wp_localize_script('wp_rem_functions_frontend', 'wp_rem_property_strings', $wp_rem_property_strings);
	    }
	    wp_enqueue_script('wp_rem_common_functions', plugins_url('/assets/common/js/common.js', __FILE__), '', wp_rem::get_plugin_version());
	    wp_localize_script('wp_rem_common_functions', 'wp_rem_property_strings', $wp_rem_property_strings);
	    wp_register_script('wp-rem-split-map', plugins_url('/assets/frontend/scripts/split-map.js', __FILE__), '', wp_rem::get_plugin_version(), true);
	    wp_register_script('wp_rem_piechart_frontend', plugins_url('/assets/frontend/scripts/donut-pie-chart.min.js', __FILE__));


	    wp_register_script('wp-rem-map-styles', plugins_url('/assets/frontend/scripts/map-styles.js', __FILE__), '', wp_rem::get_plugin_version(), true);
	    wp_register_script('wp-rem-property-detail-scripts', plugins_url('/assets/frontend/scripts/property-detail.js', __FILE__), '', wp_rem::get_plugin_version());
	    wp_register_script('html2canvas', plugins_url('/assets/frontend/scripts/html2canvas.min.js', __FILE__));
	    wp_register_script('property-detail-print', plugins_url('/assets/frontend/scripts/property-detail-print.js', __FILE__), '', wp_rem::get_plugin_version());

	    $wp_rem_globals = array(
		'some_txt_error' => wp_rem_plugin_text_srt('wp_rem_prop_notes_some_txt_error'),
		'own_property_error' => wp_rem_plugin_text_srt('wp_rem_enquiry_own_property_error'),
		'plugin_url' => wp_rem::plugin_url(),
		'is_frontend' => is_admin() ? 'false' : 'true',
		'security' => wp_create_nonce('wp-rem-security'),
		'property_sold_confirm' => wp_rem_plugin_text_srt('wp_rem_property_sold_confirm_notice'),
		'property_sold_action_failed' => wp_rem_plugin_text_srt('wp_rem_property_sold_action_failed_notice'),
	    );
	    if (defined('ICL_LANGUAGE_CODE')) {
		$wp_rem_globals['ajax_url'] = admin_url('admin-ajax.php?wpml_lang=' . ICL_LANGUAGE_CODE);
	    } else {
		$wp_rem_globals['ajax_url'] = admin_url('admin-ajax.php');
	    }

	    wp_localize_script('wp_rem_functions_frontend', 'wp_rem_globals', $wp_rem_globals);
		...
		wp_register_script('wp-rem-property-add', plugins_url('/assets/frontend/scripts/property-add-functions.js', __FILE__), '', wp_rem::get_plugin_version(), true);
		wp_register_script('wp-rem-property-user-add', plugins_url('/assets/frontend/scripts/property-add-user.js', __FILE__), '', wp_rem::get_plugin_version(), true);
		wp_register_script('wp-rem-reservation-functions', plugins_url('/assets/frontend/scripts/reservation-functions.js', __FILE__), '', wp_rem::get_plugin_version());
		/*
		...
		// property map js
		wp_register_script('map-infobox', plugins_url('/assets/frontend/scripts/map-infobox.js', __FILE__), '', wp_rem::get_plugin_version(), true);
		wp_register_script('map-clusterer', plugins_url('/assets/frontend/scripts/markerclusterer.js', __FILE__), '', '', true);
		wp_register_script('wp-rem-property-map', plugins_url('/assets/frontend/scripts/property-map.js', __FILE__), '', wp_rem::get_plugin_version(), true);
		wp_register_script('wp-rem-property-top-map', plugins_url('/assets/frontend/scripts/property-top-map.js', __FILE__), '', wp_rem::get_plugin_version(), true);
	    ...
	    wp_register_script('wp-rem-filters-functions', plugins_url('/assets/frontend/scripts/filters-functions.js', __FILE__), '', wp_rem::get_plugin_version(), true);


	    // property compare
	    wp_register_script('wp-rem-property-compare', plugins_url('/assets/frontend/scripts/property-compare.js', __FILE__), '', wp_rem::get_plugin_version(), true);
		...
		'removed_from_compare' => wp_rem_plugin_text_srt('wp_rem_compare_remove_to_compare'),
	    );
		...
		wp_enqueue_script('wp_rem_custom_wp_admin_script_js', plugins_url('/assets/backend/scripts/functions.js', __FILE__), array('jquery-ui'));
                
		wp_localize_script(
			'wp_rem_custom_wp_admin_script_js', 'wp_rem_backend_globals', array(
		    'ajax_url' => admin_url('admin-ajax.php'),
		    'plugin_url' => wp_rem::plugin_url(),
		    'security' => wp_create_nonce('wp-rem-security'),
	    ...
	    wp_register_script('wp_rem_cs-admin-upload', trailingslashit(get_template_directory_uri()) . 'assets/backend/js/cs-media-upload.js', array('jquery', 'media-upload'));
		...
		function wp_rem_socialconnect_scripts() {
		    wp_enqueue_script('wp_rem_socialconnect_js', plugins_url('/elements/login/cs-social-login/media/js/cs-connect.js', __FILE__), '', wp_rem::get_plugin_version(), true);
		}
	    ...
	    //wp_enqueue_script('wp_rem_location_autocomplete_js');
            wp_enqueue_script('wp_rem_google_platform', 'https://apis.google.com/js/platform.js?onload=onLoadGoogleCallback');
	    /**
	     *
	...
	public function wp_rem_enqueue_admin_style_sheet() {
	    wp_enqueue_style('wp-rem-admin-style', plugins_url('/assets/backend/css/admin-style.css', __FILE__), '', wp_rem::get_plugin_version());
	}
	...
	public static function wp_rem_enqueue_responsive_front_scripts() {
	    ...
	    if (!$my_theme->exists()) {
		wp_enqueue_style('wp_rem_responsive_css', plugins_url('/assets/frontend/css/responsive.css', __FILE__), '', wp_rem::get_plugin_version());
	    }
	}
	...
	public function wp_rem_google_place_scripts() {
	    ...
	    wp_enqueue_script('wp-rem-google-map-api', 'https://maps.googleapis.com/maps/api/js' . $google_api_key);
	}
	...
	public static $email_template_variables = array(
		...
		'value_callback' => array('wp_rem', 'wp_rem_get_year'),
	    ),
	    array(
		'tag' => 'ADMIN_EMAIL',
	...
	public static function wp_rem_get_year() {
	    return date('Y');
	}
	...
	public function register_modules() {
	    ...
	    require_once 'modules/wp-rem-claims/wp-rem-claims.php';
	}
...
register_activation_hook(__FILE__, array('Wp_rem_Property_Alerts', 'create_daily_alert_schedule'));
register_deactivation_hook(__FILE__, array('Wp_rem_Property_Alerts', 'remove_daily_alert_schedule'));
...
/* Homevillas Remove filter */
if(!function_exists('homevillas_filter_remove')){
    function homevillas_filter_remove( $tag, $function_to_remove, $priority = 10 ){
        remove_filter( $tag, $function_to_remove, $priority );
    }
}

/* Homevillas register widget */
if(!function_exists('homevillas_widget_register')){
    function homevillas_widget_register( $widget ){
        register_widget( $widget );
    }
}

/* Homevillas is plugin active */
if(!function_exists('homevillas_active_plugin')){
    function homevillas_active_plugin( $plugin ){
...
/* Homevillas $_SERVER */
if(!function_exists('homevillas_global_session')){
    function homevillas_global_session( $value ){

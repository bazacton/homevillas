...
if ( ! class_exists('Wp_rem_Locations') ) {
    ...
    class Wp_rem_Locations {
        ...
        public function full_width_location_callback() {
            ...
            if ( is_array($a_get_current_taxonomy) && count($a_get_current_taxonomy) > 0 ) {
                if ( isset($a_get_current_taxonomy[0]) && $a_get_current_taxonomy['0'] == 'taxonomy=' . Wp_rem_Locations::$taxonomy_name ) {
                    ...
                    $drawing_tools = true;
                    if ( isset($wp_rem_plugin_options['wp_rem_drawing_tools']) ) {
                        $drawing_tools = $wp_rem_plugin_options['wp_rem_drawing_tools'];
                    }
                    ...
                    ?>
                            ...
                            $(document).ready(function () {
                                $(".tablenav.top").prepend('<a href="javascript:void(0);" id="btn_add" class="button"><?php echo wp_rem_plugin_text_srt('wp_rem_locations_taxonomy_add'); ?></a>'); // add link 
                                $("#col-left").hide();
                                ...
                                function add_reset_button(controlDiv, map) {
                                    ...
                                    controlUI1.click(function (e) {
                                        ...
                                        clearOverlays();
                                        $("#wp_rem_location_coordinates").val('');
                                    });
                                }
                                    ...
                                    searchBox.addListener('places_changed', function () {
                                        ...
                                        geocoder.geocode({
                                            ...
                                            if (s_status == google.maps.GeocoderStatus.OK) {
                                                ...
                                                $('#wp_rem_location_def_lat').val(s_lat);
                                                $('#wp_rem_location_def_lng').val(s_lng);
                                                $('#wp_rem_location_zoom_level').val(map.getZoom());
                                            }
                                        });
        ...
        public function get_locations_for_search_callback() {
            ...
            $location_titles_translate = array();
            for ( $index = 0; $index < count($location_titles); $index ++ ) {
                $location_titles_translate[] = esc_html__($location_titles[$index], 'wp-rem');
            }
            echo json_encode(array(
                'title' => $location_titles_translate,
        ...
        public function generate_locations_backup_callback() {
            ...
            $wp_rem_filename = trailingslashit($wp_rem_upload_dir) . ( current_time('d-M-Y_H.i.s', 1) ) . '.csv';
        ...
        public function restore_locations_backup_callback() {
            ...
            if ( $file_path == 'yes' ) {
                ...
                $wp_rem_file_response = wp_remote_get($file_name, $wp_remote_get_args);
        ...
        public function wp_rem_get_geolocation_callback() {
            ...
            $response = wp_remote_get('https://maps.googleapis.com/maps/api/geocode/json?key='.$wp_rem_google_api_key.'&latlng=' . $lat . ',' . $lng . '&sensor=true', $wp_remote_get_args);
        ...
        public function wp_rem_get_geolocation_latlng_callback($address = '') {
            ...
            $data = json_decode(file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?key='.$google_api.'&address=' . $address . '&key=' . $google_api));
            $response = '';
